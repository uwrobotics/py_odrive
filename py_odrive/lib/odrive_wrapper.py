...
class OdriveEncode(CanWrapperEncode):
    ...
    def Set_Axis_Node_ID(self, joint, id):
        ...
        return self._encode(axis_id, 'Set_Axis_Node_ID', payload=payload)
    ...
    def Set_Traj_Accel_Limits(self, joint, traj_accel_limit, traj_decel_limit):
        ...
        return self._encode(axis_id, 'Set_Traj_Accel_Limits', payload=payload)
    
    def Set_Traj_Inertia(self, joint, traj_inertia):
        axis_id = self.joint2axis(joint)
        payload = {'Traj_Inertia': traj_inertia}
        return self._encode(axis_id, 'Set_Traj_Inertia', payload=payload)
    
    def Get_Iq(self, joint):
        axis_id = self.joint2axis(joint)
        return self._encode(axis_id, 'Get_Iq', rtr=True)
    
    def Get_Sensorless_Estimates(self, joint):
        axis_id = self.joint2axis(joint)
        return self._encode(axis_id, 'Get_Sensorless_Estimates', rtr=True)
    
    def Reboot(self, joint):
        axis_id = self.joint2axis(joint)
        return self._encode(axis_id, 'Reboot')
    
    def Get_Bus_Voltage_Current(self, joint):
        axis_id = self.joint2axis(joint)
        return self._encode(axis_id, 'Get_Bus_Voltage_Current', rtr=True)
    
    def Clear_Errors(self, joint):
        axis_id = self.joint2axis(joint)
        return self._encode(axis_id, 'Clear_Errors')
    
    def Set_Linear_Count(self, joint, position):
        axis_id = self.joint2axis(joint)
        payload = {'Position': position}
        return self._encode(axis_id, 'Set_Linear_Count', payload=payload)
    
    def Set_Pos_Gain(self, joint, pos_gain):
        axis_id = self.joint2axis(joint)
        payload = {'Pos_Gain': pos_gain}
        return self._encode(axis_id, 'Set_Pos_Gain', payload=payload)
    ...
    def Set_Vel_Gains(self, joint, vel_gain, vel_integrator_gain):
        ...
        return self._encode(axis_id, 'Set_Vel_Gains', payload=payload)
    
    def Get_ADC_Voltage(self, joint):
        axis_id = self.joint2axis(joint)
        return self._encode(axis_id, 'Get_ADC_Voltage', rtr=True)
    
    def Get_Controller_Error(self, joint):
        axis_id = self.joint2axis(joint)
        return self._encode(axis_id, 'Get_Controller_Error', rtr=True)
